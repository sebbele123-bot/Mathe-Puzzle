...
| `steckbrief` | `Steckbrief.jsx` | Read-only definition card; no nav button — reachable only from the Bibliothek. Asks nothing, so it grants no XP and cannot enter the rotation |

`App` routes "open from library" via `openReq` (`{definition, beweis, steckbrief, werkbank}`) + `initialId`/`taskId` props. Every `openFromLibrary` call also bumps `reqSeq`, which goes into the exercise view's `key` — so the view remounts on *every* request, even when mode and target are unchanged (the Weiter-Schleife often draws another card of the same mode, and without the key nothing would move). Independently, a view that must react to a *new* target while mounted still carries a `useEffect` on `[initialId]`/`[defId]` (`StrukturBaukasten`, `BeweisCrafter`, `Steckbrief`) — a `useState` initializer alone would keep the stale mission. The active hotbar symbol is lifted from `Inventory` to `App` via `onActive` and passed to `Werkbank` as `hand`.

Every exercise view reports back through **`onOutcome(id, fails)`**; `App.recordStat` turns that into a namespaced catalog id (`proof:`, `def:`, `sym:`), writes the rotation statistic and awards XP. **Only material that asks a question may enter the rotation** — catalog entries carry `abfragbar`, `loadRotation`/`saveRotation` filter on it (and drop entries whose catalog id no longer exists), and the Bibliothek hides the **+** on anything else. Steckbriefe (`defcard:`) are read-only, so they are `abfragbar: false`: no rotation, no `onOutcome`, no XP. A new mode that hands out XP without a gradable answer breaks that rule.

**Erstkontakt bonus:** opening a rotation card for the very first time pays `awardFirstContact` (flat 4 XP, once per catalog id, tracked in `xp.seen`) — reading a new card is the first contact and worth something. `App.openFromLibrary` takes the catalog id as its third argument and hands it to `noteFirstContact`, which pays out only for ids currently in the rotation; callers that open material (`Bibliothek`, `Training`, `nextInRotation`) must pass it. The session bar renders that award with an `erster Kontakt` label and **no** weiter button — otherwise the loop would let you skip the unsolved exercise — and fades it after 2.6 s even inside a session. The **Weiter-Schleife** (session bar above the nav, `award` state) shows the XP gained and draws the next rotation item without a detour through the Bibliothek — keep new exercise modes wired into `recordStat`, or they stay invisible to rotation and level.
...
| `src/data/catalog.js` | unifies **four** sources — structure lessons (`def:`), proof missions (`proof:`), Steckbriefe (`defcard:`), symbol tasks (`sym:`) — into one catalog | derives metadata via `parseTitle`; assigns `fach`, `typ`, `rubrik`, `abfragbar` |
